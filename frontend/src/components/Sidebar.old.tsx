...
const ReferenceItem: React.FC<ReferenceItemProps> = ({
  ...
  return (
    <div className=\"group flex items-center text-sm py-2 px-3 rounded-lg bg-gray-50 hover:bg-gray-100\">
...
const FigureItem: React.FC<FigureItemProps> = ({ figure, onPreview, onDelete }) => {
  const handleDeleteClick = (e: React.MouseEvent) => {
    e.stopPropagation();
...
export function Sidebar() {
  ...
  const [previewImage, setPreviewImage] = useState<Node | null>(null);
  ...
  // 处理编辑引用
  const handleEditReference = useCallback(async (reference: any) => {
  ...
  const handlePreviewImage = useCallback((figure: Node) => {
    setPreviewImage(figure);
    openImagePreviewModal();
  }, [openImagePreviewModal]);

  // 获取各类型节点
  const sectionNodes = nodes.filter(n => n.node_type === 'section');
  const figureNodes = nodes.filter(n => n.node_type === 'figure');
            ...
            <div className=\"space-y-1\">
              {references.map((reference) => (
                <ReferenceItem
                  ...
                  onInsertCitation={() => insertCitation(reference.title)}
                  onNavigateToCitation={() => navigateToCitation(reference.title)}
                  canInsertCitation={canInsertCitation}
                />
              ))}
            ...
            >
              {figureNodes.map((figure) => (
                <FigureItem
                  key={figure.id}
                  figure={figure}
                  onPreview={() => handlePreviewImage(figure)}
                  onDelete={() => deleteNode(figure.id)}
                />
              ))}
